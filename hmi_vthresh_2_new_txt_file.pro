...
;    endfor
    ...
    ;put power in array
    fileout[8,*] = pmap[dopptran[2,threshd_locations[*]]].data[dopptran[0,threshd_locations[*]],dopptran[1,threshd_locations[*]]]
    ;make vel_thresh string
    vtstr = straing(abs(vel_thresh[vvv]), format = '(I0)' )
    ;output file
    thresdfile = depthdir+'vthreshd-v'+vtstr+'-dopp_transients_'+directories[ddd]+'.txt'
    openw, lun, thresdfile, /get_lun, /append
 	  printf, lun, fileout
    free_lun, lun
