...
;Heinzel and Klient method for background removal and enhancement isolation
;bkremoved = total(dat[39:44,447,6])/(44.-39.) - avg(dat[39:44,447,0:5])
;absenhancement = flarebkremoved - preflarebkremoved
if (j eq 0) then begin ;initial bk subtraction
...
if (j gt 0) then begin ;accurate bk subtraction
whereslit = 0
whereslit = where(balmerdata[0, 0, i, 0:j] eq balmerdata[0, 0, i, j], ind)
wsind = array_indices(balmerdata[0, 0, i, 0:j])
dnbkb = avg(balmerdata[0, 2, wsind[*], 0:j])
endif
a = reform(a)
a = a/(44 - 39)
balmint[*,*,j] = a - dnbkb
